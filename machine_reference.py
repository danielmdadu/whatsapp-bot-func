...
# Tokens extra que puede absorber un código al extenderlo hacia adelante.
# Los sufijos de modelo son cortos ("EVO", "II", "X", "6C2"): con dos basta y
# se acota cuánto texto del lead se puede tragar el candidato.
_MAX_CODE_SUFFIX_TOKENS = 2
...
def extract_machine_code_candidate(message: Optional[str]) -> Optional[str]:
    """
    Extrae el primer código de modelo explícito que menciona el lead.

    El código NO siempre cabe en un token: muchos modelos llevan el sufijo
    separado por espacio ("C54 EVO", "S4046E II", "KTH-100 X"). Devolver solo el
    primer token producía un falso negativo grave: con "modelo C54 EVO" se
    consultaba "Simpedil C54" en Cosmos, que no existe, y el bot le decía al lead
    "no contamos con ese modelo" para 35 segundos después recomendarle esa MISMA
    máquina desde el inventario.

    Por eso el candidato se extiende hacia adelante mientras el fragmento siga
    siendo un modelo que conocemos. Solo se extiende con evidencia (el índice del
    inventario), nunca por adivinanza, así que un token siguiente cualquiera
    ("C54 para mi obra") deja el candidato intacto.
    """
    if not message:
        return None

    tokens = _raw_tokens(message)
    for i, token in enumerate(tokens):
        if not _is_code_token(_normalize(token)):
            continue

        candidato = token
        for extra in range(1, _MAX_CODE_SUFFIX_TOKENS + 1):
            if i + extra >= len(tokens):
                break
            fragmento = " ".join(tokens[i:i + extra + 1])
            if _normalize(fragmento) in _MODELS:
                candidato = fragmento
        return candidato
    return None


def segment_containing_code(message: Optional[str], code: Optional[str]) -> str:
    """
    Tramo del mensaje donde aparece `code`, para no mezclar datos de una máquina
    con los de otra.

    Un lead que pide varias máquinas las enumera en renglones o separadas por
    viñetas/"y". Con un mensaje como

        -10 Generadores Portatiles GV-8000S
        -4 Torres de Iluminación Trime X-Start

    buscar la marca en TODO el mensaje pegaba "Trime" (que es de la torre) al
    código del generador, y el bot terminaba negando un modelo que el lead nunca
    pidió: "Trime GV-8000S". Acotar la búsqueda al tramo del código evita ese
    cruce.

    Si no se puede ubicar el código, devuelve el mensaje completo (mismo
    comportamiento que antes).
    """
    if not message or not code:
        return message or ""

    # Renglones, viñetas y separadores de enumeración delimitan cada máquina.
    segmentos = [
        seg for seg in re.split(r"[\n;]+|(?:^|\s)[-•*]\s*|\s+y\s+|,\s*", message)
        if seg and seg.strip()
    ]
    objetivo = _normalize(code)
    for segmento in segmentos:
        if objetivo and objetivo in _normalize(segmento):
            return segmento
    return message
